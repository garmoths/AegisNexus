...
def subscription_upgrade(body: UpgradeRequest, api_key: APIKey = Depends(resolve_api_key), db: Session = Depends(get_db)):
    ...
    return UpgradeResponse(
        status="pending",
        message=f"Ödeme sistemi yakında aktif olacak. {price_info.get('label', body.plan.value)} plan ({price_info.get('monthly_try', '?')} TL/ay) için destek@aegisnexus.dev adresine yazın.",
