...
function SparkCard({ spark, index, reducedMotion }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 26, fontWeight: 800, color: '#fff' }}>{spark.value}</span>
        <span style={{ fontSize: 12, color: theme.textMuted }}>son 1s</span>
      </div>
      <div aria-hidden style={{ height: 2, background: `linear-gradient(90deg, ${spark.color}aa, transparent)` }} />
    </motion.div>
  )
}
...
export default function ThreatLandscapeLive() {
  ...
  return (
    <section id="threat-live" ref={containerRef} style={{ padding: '70px 24px', position: 'relative' }}>
        ...
        >
          ...
          <div style={{ display: 'grid', gap: 12, alignContent: 'start' }}>
            {threatSparks.map((spark, index) => (
              <SparkCard key={spark.label} spark={spark} index={index} reducedMotion={reducedMotion} />
            ))}
          </div>
        </div>
