  ...
  return (
    <motion.div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = `0 12px 40px ${kpi.color}18`
      }}
      onMouseLeave={(e) => {
    ...
    >
      <div
        ...
        style={{
          ...
          height: '85%',
          borderRadius: '50%',
          background: `radial-gradient(circle, ${kpi.color}22 0%, transparent 70%)`,
          filter: 'blur(28px)',
          animation: `blobDrift${index} ${8 + index * 2}s ease-in-out infinite`,
    ...
    <section id="kpi" style={{ padding: '60px 24px 80px' }}>
      ...
      <style>{`
        @keyframes blobDrift0 {
          ...
          33%  { transform: translate(14%, -10%) scale(1.08); }
          66%  { transform: translate(-6%, 12%) scale(0.94); }
        }
        @keyframes blobDrift1 {
          0%, 100% { transform: translate(0%, 0%) scale(1); }
          33%  { transform: translate(-12%, 8%) scale(1.1); }
          66%  { transform: translate(10%, -14%) scale(0.96); }
        }
        @keyframes blobDrift2 {
          0%, 100% { transform: translate(0%, 0%) scale(1); }
          33%  { transform: translate(8%, 14%) scale(0.92); }
          66%  { transform: translate(-14%, -8%) scale(1.06); }
        }
        @keyframes blobDrift3 {
          0%, 100% { transform: translate(0%, 0%) scale(1); }
          33%  { transform: translate(-8%, -12%) scale(1.04); }
          66%  { transform: translate(12%, 10%) scale(0.97); }
        }
      `}</style>
    </section>
  )
