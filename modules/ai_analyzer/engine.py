...
class AIAnalyzerEngine:
    ...
    def _generate_report(self, message: str, context: str, sender: str,
        ...
        # En kötü URL'nin skoru (ortalama degil MAX al)
        url_max = max((r.get("risk_score", 0) for r in url_results), default=0)

        # Toplam risk: MAX tabanlı, zayıf ML güçlü URL'yi ezmemeli
        total_risk = max(base_score, url_max * 0.85 + base_score * 0.15)
        ...
        # Hard override: phishing tespit edildişse minimum skor zorla
        is_phishing_flag = llm_analysis.get("is_phishing", False)
        is_scam_flag = llm_analysis.get("is_scam", False)
        if is_phishing_flag or is_scam_flag:
            total_risk = max(total_risk, 75)
        # En kötü URL yüksek riskli ise
        if url_max >= 70:
            total_risk = max(total_risk, 88)
        # Her ikisi de tetiklendiğinde → kritik
        if (is_phishing_flag or is_scam_flag) and url_max >= 70:
