...
def init_database():
    ...
    with get_db_connection() as conn:
        ...
        # Celery job sonuçları (Redis yerine SQLite — cross-process güvenli)
        cursor.execute("""
...
# ── Celery job tracking (Redis yerine SQLite — cross-process) ──────────────

def set_celery_job(job_id: str, payload: dict, ttl_seconds: int = 3600) -> bool:
    """Celery worker job sonucunu yazar. Redis job:{job_id} yerine."""
    from datetime import datetime, timedelta
    _ensure_initialized()
    try:
        ...
        with get_db_connection() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO celery_jobs (job_id, payload, expires_at) VALUES (?, ?, ?)",
                (job_id, json.dumps(payload, default=str), expires),
            )
            conn.commit()
        return True
    except Exception as e:
        logger.warning(f"set_celery_job failed for {job_id}: {e}")
        return False


def get_celery_job(job_id: str) -> Optional[dict]:
    """API worker'ın poll ettiği job sonucunu oku. Expired ise sil."""
    ...
    try:
        ...
        with get_db_connection() as conn:
            row = conn.execute(
                "SELECT payload, expires_at FROM celery_jobs WHERE job_id = ?",
                (job_id,),
            ).fetchone()
            if row is None:
                return None
            if row["expires_at"] < now:
                conn.execute("DELETE FROM celery_jobs WHERE job_id = ?", (job_id,))
                conn.commit()
                return None
            return json.loads(row["payload"])
    except Exception as e:
        logger.warning(f"get_celery_job failed for {job_id}: {e}")
        return None
