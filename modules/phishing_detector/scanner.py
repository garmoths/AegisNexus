...
try:
    if os.path.exists(PHISHTANK_PATH):
        ...
        logger.info(f"phishtank.json yüklendi: {len(PHISHTANK_DB)} domain")
    else:
        logger.warning(
            "phishtank.json bulunamadı (%s). "
            "Katman-3 devre dışı. fetch_all_sources çalıştırılarak dosya oluşturulabilir.",
            PHISHTANK_PATH,
        )
except Exception:
    pass
...
# =========================================================
# RESMİ TÜRK TLD'LERİ — ANİNDE GÜVEN
# =========================================================
# Bu TLD'ler BTK tarafından zorunlu kimlik/kurum doğrulamasıyla verilir.
# Eşleşme → tarama yapılmaz, direkt score=100 döner.
OFFICIAL_TR_TLDS = {
    ".gov.tr": "T.C. Devlet Kurumu (BTK kayıtlı)",
    ".bel.tr": "Türkiye Belediyesi (BTK kayıtlı)",
    ".pol.tr": "Emniyet Teşkilatı (BTK kayıtlı)",
    ".tsk.tr": "Türk Silahlı Kuvvetleri (BTK kayıtlı)",
    ".edu.tr": "YÖK Onaylı Üniversite (BTK kayıtlı)",
    ".k12.tr": "MEB Onaylı Okul (BTK kayıtlı)",
    ".av.tr":  "Baro Onaylı Hukuk Bürosu (BTK kayıtlı)",
    ".dr.tr":  "TTB Onaylı Sağlık Kuruluşu (BTK kayıtlı)",
}


def check_official_tr_tld(domain: str) -> dict | None:
    """
    Resmi Türk TLD kontrolü. Kesin güvenilir → score=100, tarama yapılmaz.

    Kural: domain tam olarak <isim><tld> kalıbında olmalı.
    - 'metu.edu.tr'  → TLD='.edu.tr', eşleşir ✓
    - 'enes.edus.tr' → '.edus.tr' listede yok, eşleşmez ✓
    - 'edu.tr'       → prefix boş, eşleşmez (TLD'nin kendisi) ✓
    """
    domain_lc = domain.lower().strip()
    if domain_lc.startswith("www."):
        domain_lc = domain_lc[4:]
    # port varsa at
    domain_lc = domain_lc.split(":")[0]

    for tld, label in OFFICIAL_TR_TLDS.items():
        if domain_lc.endswith(tld):
            prefix = domain_lc[: -len(tld)]  # örn 'metu' ya da 'sub.metu'
            if prefix:  # boş prefix = TLD'nin kendisi, geçersiz
                return {"trusted": True, "tld": tld, "label": label, "domain": domain_lc}
    return None
...
def run_quick_checks(input_url: str, db: Session = None) -> dict:
    ...
    # ── Katman 0+: Resmi Türk TLD — anlık güven ─────────
    _tr_tld = check_official_tr_tld(domain)
    if _tr_tld:
        return {
            "url": input_url, "safety_score": 100, "score": 100,
            "risk_level": "✅ Güvenli",
            "details": [
                f"✅ Resmi Türk devlet/kurum TLD'si tespit edildi: {_tr_tld['tld']}",
                f"   {_tr_tld['label']}",
                "   Bu TLD'ler BTK tarafından zorunlu kimlik doğrulamasıyla verilir.",
            ],
            "sources": [{"name": "Resmi TR TLD", "status": f"✅ {_tr_tld['label']}"}],
    ...
    # ── Katman 2.5: Domain DB eşleşmesi (exact URL dışı, domain bazlı) ───
    if db:
        domain_match = db.query(PhishingURL).filter(
            PhishingURL.domain_norm.in_([d for d in {domain, raw_domain} if d])
        ).first()
        if domain_match:
            return {
                "url": input_url, "safety_score": 0, "score": 0,
                "risk_level": "🚨 ÇOK TEHLİKELİ (DB Kayıtlı)",
                "details": [f"Tehlikeli domain veritabanında tespit edildi! (ID: {domain_match.phish_id})"],
                "sources": [{"name": "Internal DB", "status": "TEHDİT 🚨"}],
                "risks": ["Tehlikeli domain veritabanında kayıtlı"],
                "definitive": True, "is_whitelisted": False,
            }
...
def calculate_safety_score(input_url, db: Session = None):
    ...
    # ---------------------------------------------------------
    # 0+. KATMAN: RESMİ TÜRK TLD — ANLIK GÜVEN
    # ---------------------------------------------------------
    _tr_tld = check_official_tr_tld(domain)
    if _tr_tld:
        return {
            ...
            "sources": [{"name": "Resmi TR TLD", "status": f"✅ {_tr_tld['label']}"}],
            "risks": [], "threat_intel": None,
            "confidence": "high", "score_range": [95, 100],
            "signal_count": 1, "scoring_model": SCORING_MODEL,
        }
    ...
    # ---------------------------------------------------------
    # 3. KATMAN: PHISHTANK (JSON — in-memory)
    # ---------------------------------------------------------
    if domain in PHISHTANK_DB or raw_domain in PHISHTANK_DB:
        return {
            "url": input_url,
    ...
    # ---------------------------------------------------------
    # 3.5. KATMAN: PHISHTANK CANLI API (taze URL'ler için)
    # ---------------------------------------------------------
    try:
        from .threat_intel import check_phishtank_live
        pt_live = check_phishtank_live(input_url, timeout=5)
        if pt_live.get("in_database") and pt_live.get("valid"):
            phish_id = pt_live.get("phish_id") or "?"
            return {
                "url": input_url,
                "safety_score": 0,
                "score": 0,
                "risk_level": "🚨 ÇOK TEHLİKELİ (PhishTank Canlı)",
                "details": [
                    f"PhishTank canlı sorguda tespit edildi! (ID: {phish_id})",
                    "Kesinlikle veri girmeyin, siteyi terk edin."
                ],
                "sources": [{"name": "PhishTank Live", "status": "TEHDİT 🚨"}],
                "definitive": True,
            }
    except Exception:
        pass
...
# ---------------------------------------------------------
    ...
    if not site_is_up:
        _offline_score = max(0, min(100, score))
        if threat_result and not is_whitelisted:
            _vt = threat_result.get("virustotal") or {}
            _aipdb = threat_result.get("abuseipdb") or {}
            _vt_mal = _vt.get("malicious", 0) or 0
            _ab_score = _aipdb.get("abuse_score", 0) or 0
            if _vt_mal >= 1 and _ab_score >= 70:
                _offline_score = min(_offline_score, 20)
            elif _vt_mal >= 3:
                _offline_score = min(_offline_score, 15)
            elif _vt_mal >= 1:
                _offline_score = min(_offline_score, 35)
            elif _ab_score >= 70:
                _offline_score = min(_offline_score, 40)
        interim_classification = classify_with_confidence(_offline_score, len(sources))
        return {
            "url": input_url,
            "safety_score": _offline_score,
            "score": _offline_score,
    ...
    # Hard cap: tehdit kaynakları tespit edildiğinde güvenli skor verilemez
    if threat_result and not is_whitelisted:
        vt = threat_result.get("virustotal") or {}
        aipdb = threat_result.get("abuseipdb") or {}
        vt_malicious = vt.get("malicious", 0) or 0
        abuse_score = aipdb.get("abuse_score", 0) or 0
        if vt_malicious >= 1 and abuse_score >= 70:
