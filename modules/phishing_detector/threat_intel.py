...
def _screenshot_failed_penalty(current_risk_score: int) -> int:
    ...
    - risk<20: 10 (belirsizlik — sıfır skorda da uygula)
    - 20<=risk<50: 15
    - risk>=50: 20
    """
...
# ─── Bilinen ücretsiz hosting platformları (phishing sıklıkla burada barınır) ───
_FREE_HOSTING_PLATFORMS: frozenset = frozenset({
    "weebly.com", "wordpress.com", "blogspot.com", "github.io",
    "wix.com", "squarespace.com", "jimdo.com", "yolasite.com",
    "webnode.com", "strikingly.com", "site123.com", "webflow.io",
    "netlify.app", "vercel.app", "pages.dev", "render.com",
    "000webhostapp.com", "byethost.com", "infinityfree.net",
    "bsite.net", "freewebhostingarea.com", "hostfree.pw",
    "glitch.me", "replit.dev", "carrd.co",
})

# Phishing sıklıkla subdomain/yol içinde bu marka kelimelerini kullanır
_BRAND_KEYWORDS: frozenset = frozenset({
    "paypal", "microsoft", "apple", "google", "amazon", "netflix",
    "facebook", "instagram", "twitter", "whatsapp", "tiktok",
    "bank", "banka", "hsbc", "barclays", "lloyds", "chase",
    "wellsfargo", "citibank", "boa", "ziraat", "garanti", "akbank",
    "isbank", "vakifbank", "halkbank", "yapikredi",
    "ebay", "alibaba", "shopify", "coinbase", "binance",
    "verify", "secure", "login", "signin", "account", "update",
    "confirm", "suspend", "alert", "billing", "invoice",
})


def _check_freehost_phishing(url: str, domain: str) -> int:
    """
    URL ücretsiz bir hosting platformunda barınıyorsa VE subdomain/yol
    bilinen bir marka/phishing anahtar kelimesi içeriyorsa ceza döndür.
    Gerçek phishing örneği: microsoft761login.weebly.com
    Döndürür: ceza puanı (0, 25 veya 35)
    """
    domain_lc = (domain or "").lower()
    url_lc = (url or "").lower()

    # Platform tespiti: domain veya üst domain free hosting listesinde mi?
    matched_platform = None
    for platform in _FREE_HOSTING_PLATFORMS:
        if domain_lc == platform or domain_lc.endswith("." + platform):
            matched_platform = platform
            break
    if not matched_platform:
        return 0

    # Platform eşleşti — subdomain veya URL yolunda marka kelimesi var mı?
    subdomain = domain_lc.replace("." + matched_platform, "").replace(matched_platform, "")
    search_text = subdomain + " " + url_lc
    matched_brands = [kw for kw in _BRAND_KEYWORDS if kw in search_text]
    if not matched_brands:
        return 0

    # Sadece action/phishing keyword (verify, login vb.) → 25 ceza
    # Gerçek bir marka ismi (microsoft, paypal vb.) → 35 ceza
    _action_only = frozenset({"verify", "secure", "login", "signin", "account",
                               "update", "confirm", "suspend", "alert", "billing", "invoice"})
    real_brand_hit = any(kw not in _action_only for kw in matched_brands)
    penalty = 35 if real_brand_hit else 25
    logger.info(
        "[FreeHost] %s ücretsiz hosting + marka '%s' tespit edildi → ceza +%d",
        domain, matched_brands[:3], penalty,
    )
    return penalty
...
def _task_virustotal(url: str):
    """VirusTotal sorgusu — API key varsa gerçek VT API, yoksa yerel DB sorgusu."""
    # API keyi varsa gerçek VirusTotal API'sini kullan
    if VIRUSTOTAL_API_KEYS:
        try:
            result = check_virustotal(url, timeout=10)
            if result.get("available"):
                return result
            # API ulaşılamıyorsa (429, 401 vb.) yerel DB'ye fall back et
            logger.warning(f"VT API kullanılamıyor ({result.get('status')}), yerel DB'ye geçiliyor: {url}")
        except Exception as exc:
            logger.warning(f"VT API hatası, yerel DB'ye geçiliyor: {exc}")

    # API key yok veya API başarısız → yerel DB sorgusu
    from .threat_intel_local import check_virustotal_local
    from app.database import SessionLocal
    db = None
    try:
        db = SessionLocal()
        result = check_virustotal_local(url, db)
        return result
    except Exception as exc:
...
def _task_abuseipdb(url: str):
    ...
    """
    from .threat_intel_local import (
        _best_cached_scan_raw_data,
        _extract_cached_component,
        check_abuseipdb_local as check_abuseipdb,
    )
    ...
    if cached_scan:
        ...
        if isinstance(cached_abuse, dict):
            ...
            result.setdefault("status", "cache-hit")
            result["source"] = result.get("source") or "cache"
            return result
...
def _task_spamhaus_group(url: str, domain: str, resolved_ip: str | None):
    ...
    from .threat_intel_local import _best_cached_scan_raw_data, _extract_cached_component
    ...
    cached_scan = _best_cached_scan_raw_data(url, domain)
    if cached_scan:
        cached_domain = _extract_cached_component(cached_scan, "spamhaus_domain")
        cached_ip = _extract_cached_component(cached_scan, "spamhaus_ip")
        if isinstance(cached_domain, dict) or isinstance(cached_ip, dict):
            group: dict = {}
            if isinstance(cached_domain, dict):
                group["spamhaus_domain"] = cached_domain
            if isinstance(cached_ip, dict):
                group["spamhaus_ip"] = cached_ip
            if not group:
                group = {}
            return group
...
def run_threat_intelligence(url, http_meta=None, page_text: str = "", is_whitelisted: bool = False, pre_penalty: int = 0):
    ...
    # --- Ücretsiz Hosting + Marka Phishing Heuristic ---
    if not is_whitelisted:
        freehost_penalty = _check_freehost_phishing(url, domain)
        if freehost_penalty > 0:
            results["total_penalty"] += freehost_penalty
            results["sources"].append({
                "name": "FreeHost Heuristic",
                "status": f"Ücretsiz hosting platformunda marka/kimlik avı deseni (+{freehost_penalty})"
            })
            results["findings"].append(
                f"⚠️ FreeHost Heuristic: Ücretsiz hosting + marka kelimesi tespit edildi (ceza +{freehost_penalty})"
            )
...
# =========================================================
# PHISHTANK CANLI KONTROL (checkurl API)
# =========================================================

def check_phishtank_live(url: str, timeout: int = 5) -> dict:
    """PhishTank checkurl API ile gerçek zamanlı kontrol.

    Sync (12 saatlik) DB'de olmayan taze URL'leri yakalar.
    API key opsiyoneldir; anonim istekler de çalışır (rate-limited).
    Döndürür: {"in_database": bool, "valid": bool, "phish_id": str|None, "error": str|None}
    """
    ...
    try:
        ...
        resp = requests.post(
            ...
            data=payload,
            headers={"User-Agent": f"phishtank/{os.getenv('PHISHTANK_USERNAME', 'aegisnexus')}"},
            timeout=timeout,
        )
        if resp.status_code != 200:
            return {"in_database": False, "valid": False, "phish_id": None, "error": f"HTTP {resp.status_code}"}
        data = resp.json()
        result = data.get("results", {})
        return {
            "in_database": bool(result.get("in_database", False)),
            "valid": bool(result.get("valid", False)),
            "phish_id": result.get("phish_id"),
            "error": None,
        }
    except Exception as exc:
        logger.debug(f"[PhishTank-Live] {url} kontrol hatası (tolere edildi): {exc}")
        return {"in_database": False, "valid": False, "phish_id": None, "error": str(exc)}
