...
def _http_session() -> requests.Session:
    ...
    session.headers.update({"User-Agent": f"phishtank/{pt_username}"})
    return session
...
def _fetch_phishtank_direct(http: requests.Session) -> List[Dict[str, Any]]:
    """PhishTank JSON DB'sini doğrudan indirir. App key varsa limitsiz, yoksa günde birkaç kez."""
    import bz2
    ...
    try:
        ...
        print(f"PhishTank HTTP {resp.status_code}")
    except Exception as exc:
        print(f"PhishTank indir hatası: {exc}")
    return []


def update_phishtank_json(session: Optional[requests.Session] = None) -> int:
    """PhishTank JSON DB'sini (veya OpenPhish fallback) phishtank.json olarak kaydeder.
    - PHISHTANK_APP_KEY tanımlıysa → PhishTank'tan doğrudan indirir (limitsiz)
    - Tanımlı değilse → OpenPhish feed fallback
    Döndürür: kaydedilen URL sayısı.
    """
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_path = os.path.join(BASE_DIR, "phishtank.json")
    http = session or _http_session()
    ...
    if app_key:
        print("   [PhishTank] App key bulundu, doğrudan indiriliyor...")
        pt_data = _fetch_phishtank_direct(http)
        if pt_data:
            ...
            print(f"   [PhishTank] {len(entries)} URL indirildi")
        else:
            print("   [PhishTank] Veri alınamadı, OpenPhish fallback deneniyor...")

    if not entries:
        urls: List[str] = []
        try:
            resp = http.get("https://openphish.com/feed.txt", timeout=60)
            if resp.status_code == 200:
                urls = parse_feed_lines_to_urls(resp.text)
        except Exception as exc:
            print(f"OpenPhish indir hatası: {exc}")
        if not urls:
            return 0
        now = datetime.now(timezone.utc).isoformat()
        entries = [{"url": u, "submission_time": now, "status": "valid"} for u in urls]
    try:
        with open(output_path, "w", encoding="utf-8") as fh:
            json.dump(entries, fh)
        # scanner.py PHISHTANK_DB'yi sıcak güncelle
        try:
            from . import scanner as _scanner
            for e in entries:
                u = e["url"]
                clean = u.replace("https://", "").replace("http://", "").replace("www.", "").split("/")[0]
                _scanner.PHISHTANK_DB.add(clean)
        except Exception:
            pass
        print(f"phishtank.json güncellendi: {len(entries)} URL → {output_path}")
        return len(entries)
    except Exception as exc:
        print(f"phishtank.json yazma hatası: {exc}")
        return 0
...
def _parse_bool(val) -> bool:
    """'yes'/'true'/'1'/True → True, diğerleri → False."""
    if isinstance(val, bool):
        return val
    return str(val).strip().lower() in ("yes", "true", "1", "online")
...
def import_to_database(db: Session, entries: List[Dict[str, Any]], batch_size: int = 1000) -> Dict[str, int]:
    ...
    for i, entry in enumerate(entries):
        try:
            ...
            if url_hash in batch_seen:
                continue
            batch_seen.add(url_hash)

            canonical_url = (normalized.get("canonical_url") or url)[:_URL_MAX]
            domain_norm = (normalized.get("domain_norm") or "")[:_DOMAIN_MAX]

            existing_by_hash = db.query(PhishingURL).filter(PhishingURL.url_hash == url_hash).first()
            if existing_by_hash:
                existing_by_hash.url = canonical_url
                existing_by_hash.domain_norm = domain_norm
                existing_by_hash.status = entry.get("status", "unknown")
                existing_by_hash.online = _parse_bool(entry.get("online", False))
                ...
                if existing_by_id:
                    existing_by_id.url = canonical_url
                    existing_by_id.url_hash = url_hash
                    existing_by_id.domain_norm = domain_norm
                    existing_by_id.status = entry.get("status", "unknown")
                    existing_by_id.online = _parse_bool(entry.get("online", False))
                ...
                db.add(
                    PhishingURL(
                        ...
                        online=_parse_bool(entry.get("online", False)),
...
def fetch_all_sources(db: Session) -> Dict[str, Any]:
    ...
    # phishtank.json güncelle (OpenPhish → disk + bellek)
    print("\n🗂️  phishtank.json yenileniyor...")
    try:
        pt_count = update_phishtank_json(session=session)
        if pt_count:
            print(f"   ✅ phishtank.json: {pt_count} URL")
        else:
            print("   ⚠️  phishtank.json güncellenemedi (OpenPhish erişilemez olabilir)")
    except Exception as pt_exc:
        print(f"   ⚠️  phishtank.json hata: {pt_exc}")
