...
_NETWORK_IDLE_REQUIRED = os.getenv("SCREENSHOT_NETWORK_IDLE", "true").strip().lower() in {
    "1", "true", "yes", "on"
}
_NETWORK_IDLE_TIMEOUT_MS = int(os.getenv("SCREENSHOT_NETWORK_IDLE_TIMEOUT_MS", str(PLAYWRIGHT_TIMEOUT_MS)))
...
def _capture_screenshot_base64(url: str, page_text: str) -> Tuple[str, str, bool]:
    ...
    Strateji:
    1. Varsayılan olarak networkidle ile bekler (SCREENSHOT_NETWORK_IDLE=true).
       networkidle timeout olursa kontrollü şekilde domcontentloaded fallback kullanır.
    ...
    for attempt in range(1, _SCREENSHOT_MAX_RETRIES + 1):
        try:
            with acquire_browser_context() as context:
                ...
                if _NETWORK_IDLE_REQUIRED:
                    try:
                        page.goto(url, wait_until="networkidle", timeout=_NETWORK_IDLE_TIMEOUT_MS)
                    except PlaywrightTimeoutError:
                        logger.debug(f"networkidle timeout, domcontentloaded fallback: {url}")
                ...
                # Ek güvenlik: DOM tamamen yüklenmeden screenshot alma
                try:
                    page.wait_for_function("document.readyState === 'complete'", timeout=4000)
                except Exception:
                    pass
