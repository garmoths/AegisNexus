...
def get_attack_trends(
    ...
    with _session() as db:
        query = (
            db.query(
                ...
                func.count(VictimCase.id).label("cnt"),
            )
            .filter(
                VictimCase.last_seen >= cutoff,
                VictimCase.is_published == True,
            )
        )
        if region:
            query = query.filter(VictimCase.region == region)

        rows = (
            query.group_by(VictimCase.attack_method)
            .order_by(desc("cnt"))
            .limit(top_n)
            .all()
        )
    ...
    for rank, row in enumerate(rows, start=1):
        method = row.attack_method or "unknown"
        results.append(
            {
                "attack_method": method,
                "attack_method_tr": _attack_method_tr(method),
                "count": row.cnt,
