...
from .database import ensure_initialized, get_attack_trends, get_case, get_cases, get_ingest_health, get_stats
...
def attack_trends(
    ...
    """
    Son N günde en çok kullanılan saldırı yöntemlerini döner.

    Örnek sorular:
    - 'Son 30 günde İstanbul'da en çok hangi dolandırıcılık yöntemi kullanıldı?'
    - 'Bu ay Türkiye genelinde kaç phishing vakası var?'

    Query params:
    - days: Kaç günlük periyot (varsayılan: 30)
    - region: Şehir filtresi, ör. 'İstanbul' (opsiyonel)
    - top_n: Kaç yöntem dönsün (max 10)
    """
    top_n = min(max(1, top_n), 10)
    days = min(max(1, days), 365)

    trends = get_attack_trends(days=days, region=region, top_n=top_n)
    ...
    return {
        ...
        "top_n": top_n,
        "trends": trends,
        "total_cases_in_top": sum(t["count"] for t in trends),
        "module": "07_victim_atlas",
    }


# ── Fraud Profile Endpoints ────────────────────────────────


class FraudProfileMatchRequest(BaseModel):
    text: str
    attack_method: str = ""


@router.get("/fraud-profiles")
def list_fraud_profiles(db: Session = Depends(get_db)):
    """Tüm dolandırıcılık profillerini listele."""
    from app.models import FraudProfile
    from .fraud_profile_matcher import seed_profiles

    profiles = db.query(FraudProfile).order_by(FraudProfile.severity.desc(), FraudProfile.case_count.desc()).all()
    if not profiles:
        seed_profiles(db)
        profiles = db.query(FraudProfile).order_by(FraudProfile.severity.desc()).all()
...
@router.post("/fraud-profiles/match")
def match_fraud_profile(body: FraudProfileMatchRequest, db: Session = Depends(get_db)):
    """Metin içeriğini fraud profilleriyle eşleştir."""
    from .fraud_profile_matcher import match_profile

    if not body.text or len(body.text.strip()) < 10:
        raise HTTPException(status_code=422, detail="Analiz için yeterli metin gerekli (min 10 karakter)")

    matched = match_profile(
        text=body.text,
        attack_method=body.attack_method or None,
        db=db,
    )
...
@router.get("/fraud-profiles/{slug}")
def get_fraud_profile(slug: str, db: Session = Depends(get_db)):
    """Belirli bir fraud profilini ve son 5 benzer vakayı döner."""
    from app.models import FraudProfile

    profile = db.query(FraudProfile).filter(FraudProfile.slug == slug).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Profil bulunamadı")

    recent_cases = []
    if profile.attack_method:
        from .database import get_cases
        cases_result = get_cases(db=db, attack_method=profile.attack_method, limit=5, page=1)
        recent_cases = [
            {
                "id": c["id"],
            ...
            for c in (cases_result.get("cases") or [])
        ]
    ...
    return {
        ...
        "data": {
            ...
            "attack_method": profile.attack_method,
            "severity": profile.severity,
            "case_count": profile.case_count,
            "avg_loss_try": profile.avg_loss_try,
